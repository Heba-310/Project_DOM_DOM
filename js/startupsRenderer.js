  ...
  return `
    <article class="ex-card${highlightClass}">
      ...
      <div class="ex-card__footer">
        ...
        <button class="ex-btn-review" data-slug="${slug}" data-id="${startup.id}" onclick="goToProfile('${slug}')">
          Review Deal
...
function goToProfile(slug) {
  window.location.href = './startupProfile.html?slug=' + encodeURIComponent(slug);
}
...
async function loadStartups() {
  ...
  try {
    ...
    console.error(err);
    renderError("Failed to load startups. Please try again.");
  }
}
